...
import useCurrentUser from "@/hooks/useCurrentUser";
import { Toaster, toast } from "sonner";
import { initializeFCM, sendPushNotification } from "@/services/notificationService";
...
export default function JobDetailsPage() {
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      const companyUser = users.find((u) => u.id === job.companyId);
      if (companyUser) {
        await addDoc(collection(db, "notifications"), {
          recipientId: job.companyId,
          senderId: userId,
          message: `${currentUser.displayName || "A user"} applied to your job "${job.title}"`,
          createdAt: new Date(),
          read: false,
          type: "application",
          relatedId: id,
        ...
        const fcmToken = companyUser.fcmToken;
        if (fcmToken) {
          await sendPushNotification({
            token: fcmToken,
            title: "New Job Application",
            body: `${currentUser.displayName || "A user"} applied to ${job.title}`,
            data: { jobId: id },
          });
        }
      }
