      ...
      <div className="relative z-10">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
                ...
                {rating && (
                  <div className="flex items-center gap-2">
                    <div className="flex text-yellow-400">
                      {[...Array(5)].map((_, i) => (
                        <span key={i} className={i < Math.floor(rating) ? "text-yellow-400" : "text-slate-300"}>
                          ★
                        </span>
                      ))}
                    </div>
              ...
              {!isOwner && (
                <button
                  ...
                  className="flex gap-2 items-center px-5 py-2.5 bg-primary text-white rounded-xl hover:bg-primary/90 transition-all duration-200 shadow-lg hover:shadow-xl font-medium"
                >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="text-center p-3 bg-slate-50 rounded-2xl border border-slate-200">
            ...
            <p className="text-base font-semibold text-primary">{mainTrack || "Not specified"}</p>
          </div>
          <div className="text-center p-3 bg-slate-50 rounded-2xl border border-slate-200">
            <p className="text-sm text-slate-600 mb-1">Current Job</p>
            <p className="text-base font-semibold text-primary">
              {typeof currentJob === "string" && currentJob ? currentJob : "Available"}
            </p>
          </div>
          <div className="text-center p-3 bg-slate-50 rounded-2xl border border-slate-200">
            <p className="text-sm text-slate-600 mb-1">Status</p>
            <p className="text-base font-semibold text-green-600">{status || "Active"}</p>
          </div>
        </div>
