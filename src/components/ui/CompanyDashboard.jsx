  ...
  const fetchFreelancers = async () => {
    ...
    try {
      ...
      querySnapshot.forEach((docSnap) => {
        ...
        console.log("Job:", job.title, job.applicants); // 🔍 نطبع هنا كل applicants

        const applicants = Array.isArray(job.applicants) ? job.applicants : [];

        applicants.forEach((applicant) => {
          console.log("Applicant:", applicant); // 🔍 نتأكد من البنية
    ...
    try {
      ...
      jobsSnap.forEach((jobDoc) => {
        ...
        if (jobData.applicants && jobData.applicants.length > 0) {
          // يتأكد إن فيه متقدم واحد Approved
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      try {
        ...
        const rows = await Promise.all(
          Array.from(applicantsMap.values()).map(async (p) => {
    ...
    <div className="min-h-screen bg-[#f9f9f9]">
      ...
      <main className="p-6 max-w-7xl mx-auto">
        ...
        <p className="text-gray-600 mb-6">
          Welcome back! Here's what's happening with your ITI freelance platform.
        </p>
...
<section className="bg-white shadow p-4 rounded">
  ...
  {Object.keys(applicationsByJob).length > 0 ? (
    ...
    <p className="text-gray-500 text-sm">No applications yet.</p>
          ...
          {/* ITI Graduates Section with scroll */}
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 mt-1">
                      <span className="flex items-center text-yellow-500 text-sm">
                        <Star className="w-4 h-4 mr-1" /> {grad.rating}
                      </span>
